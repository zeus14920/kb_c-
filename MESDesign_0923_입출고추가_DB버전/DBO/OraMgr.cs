    ...
    class OraMgr
    {
        ...
        public void getMaterialVo(out List<Material> list, out List<MaterialAddSub> addSubList)
        {
            string sql = "select m.mcode as 코드,m.mname as 제품명, " +
                ...
                "i.time as 자재입출고시간 from import i, warehouse w, material m " +
            ...
            OracleDataReader rd = cmd.ExecuteReader();
            //List<Material> list = new List<Material>();
            //List<MaterialAddSub> addSubList = new List<MaterialAddSub>();
            ...
            if (rd.HasRows)
            {
                while (rd.Read())
                {
                    ...
                    MaterialAddSub matAddSub = new MaterialAddSub
                        (rd["입고수량"].ToString(), null, rd["재고수량"].ToString(), rd["자재입출고시간"].ToString());
